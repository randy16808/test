...
`timescale 1 ns / 1 ps
	...
	//reg  [31:0] sram[15:0];
	reg [C_M_AXI_DATA_WIDTH-1:0] sram [0:C_M_AXI_BURST_LEN-1];
	wire we, en;
	reg  [31:0] data_out;
	wire [31:0] data_in;
	//wire [C_TRANSACTIONS_NUM:0] sram_addr;
	assign en = rnext | (mst_exec_state[1] & ~mst_exec_state[0]);
	assign wn = mst_exec_state & 2'b10;
	assign data_in = M_AXI_RDATA;
	//assign sram_addr = (mst_exec_state & 5'b11101 & read_index)|(mst_exec_state & 5'b11110 & write_index);
	  ...
	  begin
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)
	      axi_wdata <= 'b0;
	        ...
	        axi_wdata <= (wnext)? buffer[write_index+1] : buffer[0];
	    else
	        axi_wdata <= axi_wdata;
	  end
	*/
	...
	always @(posedge M_AXI_ACLK)
	  ...
	  */
	always @(posedge M_AXI_ACLK) begin // Write data into the SRAM block
		if (en) begin
		sram[read_index] <= M_AXI_RDATA;
		end
	end
	
	always @(posedge M_AXI_ACLK) begin // Read data from the SRAM block
		if (en && we) // If data is being written into SRAM,
		axi_wdata <= axi_wdata; // forward the data to the read port
		else
		axi_wdata <= sram[write_index]; // Send data to the read port
	end
